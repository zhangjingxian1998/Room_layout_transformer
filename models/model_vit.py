...
class ViTSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False,whole_mask=None,use_causal_attention=None):
        ...
        # attention_probs.shape = torch.Size([1, 12, 50, 50])           ####????????[(1,12,197,197)]?????
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)   ##取前两个维度[1,197]并加上最后一个维度768——[1,197,768]
...
class ViTIntermediate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = self.dense(hidden_states)   ###dense 全连接层
...
class ViTLayer(nn.Module):
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False,whole_mask=None,use_causal_attention=None):
        ...
        # second residual connection is done here 残差
...
class ViTModel(ViTPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        ...
        # ViTPooler模块的全连接层只接收ViTEncoder模块中第一个token（[CLS]）对应的输出
        # [CLS] 信息也传输给prompt来表示图片全局信息
